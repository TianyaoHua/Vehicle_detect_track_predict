...
double calStd(CMatrix<float> matrix){
...
// writeTracks
void writeTracks(int startFrame) {
  char buffer[50];
  ...
  for (int i = 0; i < mTracks.size(); i++) {
    ...
    aFile << mTracks[i].mox << " " << mTracks[i].moy << " " << mTracks[i].mSetupTime + startFrame << std::endl;
    for (int j = 0; j < aSize; j++)
      aFile << mTracks[i].mx[j] << " " << mTracks[i].my[j] << " " << mTracks[i].mu[j] << " " << mTracks[i].mv[j] << " " << mTracks[i].mcolor[j][0] << " " << mTracks[i].mcolor[j][1] << " " << mTracks[i].mcolor[j][2] << " " << mTracks[i].mSetupTime+j+1+startFrame << std::endl;
  }
}
...
//    CTensor<float> aShow(*aImage2);
    ...
    double sigmaU = calStd(flowU);
    double sigmaV = calStd(flowU);
    ...
    for (unsigned int i = 0; i < mTracks.size(); i++) {
      ...
      if (aUnreliable(iax,iay) > 0) mTracks[i].mStopped = true;
        ...
        if (ibx < 0 || iby < 0 || ibx >= mXSize || iby >= mYSize) mTracks[i].mStopped = true;
          ...
          // Add mu, mv, mcolor. Save a->b forward flow value.
          mTracks[i].mu.push_back(aForward(iax,iay,0));
          mTracks[i].mv.push_back(aForward(iax,iay,1));
          
          std::vector<float> mcolor;
          mcolor.push_back((*aImage2)(ibx, iby, 0));
          mcolor.push_back((*aImage2)(ibx, iby, 1));
          mcolor.push_back((*aImage2)(ibx, iby, 2));
          // rgb2lab(mcolor);
          mTracks[i].mcolor.push_back(mcolor);
